        ...
        <div class="col-12">
            @component('components.backend.card.card-form')
                ...
                @slot('content')
                    ...
                    <x-forms.input type="number" name="sale_price" id="sale_price" :label="__('Harga Jual')" :isRequired="true" value="{{ $data['product']->sale_price }}"/>

                    <x-forms.input type="number" name="weight" id="weight" :label="__('field.weight')" :isRequired="true" value="{{ $data['product']->weight }}"/>

                    <x-forms.select name="discount" id="discount" label="{{ __('button.select') }} {{ __('menu.category') }}" :isRequired="true">                        
                            <option value="{{ $data['product']->discount }}" selected>{{ $data['product']->discount }}</option>
